...
modify_container_interactive() {
    ...
    # 修复飞牛 OS 等系统上卷名前的容器名前缀（如 "g-box:卷ID" -> "卷ID"）
    sed -i -E 's/(-v[[:space:]]+|--volume[[:space:]]+)"[^:]+:([a-f0-9]{64})"/\1"\2"/g' "$runlike_file"
...
rebuild_container() {
    ...
    if bash "${config_file}"; then
        # 如果是 g-box 容器，生成 diy_mount.txt
        if [[ "$container_name" == "g-box" ]] || [[ "$container_name" =~ g-?box ]]; then
            INFO "检测到 G-Box 容器，正在生成 diy_mount.txt..."

            # 从容器获取 /data 对应的宿主机路径
            local config_dir=$(docker inspect -f '{{range .Mounts}}{{if eq .Destination "/data"}}{{.Source}}{{end}}{{end}}' "${container_name}" 2>/dev/null)

            if [ -n "$config_dir" ] && [ -d "$config_dir" ]; then
                local diy_mount_file="${config_dir}/diy_mount.txt"

                # 保留的容器路径（系统挂载，不写入 diy_mount.txt）
                local reserved_dests=("/data" "/www/data" "/var/run/docker.sock")

                # 清空 diy_mount.txt
                > "$diy_mount_file"

                # 使用 docker inspect 获取所有 bind 类型的挂载
                # 使用 bash -c 确保管道正确处理，tr '|' '\n' 将 | 换成换行
                local mounts_output=$(docker inspect -f '{{range .Mounts}}{{if eq .Type "bind"}}{{.Source}}:{{.Destination}}|{{end}}{{end}}' "${container_name}" 2>/dev/null | tr '|' '\n')

                # 确保使用 bash 来处理（避免 zsh 管道问题）
                echo "$mounts_output" | while IFS= read -r mount_info; do
                    [[ -z "$mount_info" ]] && continue

                    # 提取容器路径（第二个字段）
                    local container_path=$(echo "$mount_info" | cut -d':' -f2)

                    # 检查是否是保留路径
                    local is_reserved=false
                    for reserved in "${reserved_dests[@]}"; do
                        if [[ "$container_path" == "$reserved" ]]; then
                            is_reserved=true
                            break
                        fi
                    done

                    # 如果不是保留路径，写入 diy_mount.txt
                    if [ "$is_reserved" = false ]; then
                        # 只取前两个字段（宿主机路径和容器路径），用空格分隔
                        local host_path=$(echo "$mount_info" | cut -d':' -f1)
                        echo "$host_path $container_path" >> "$diy_mount_file"
                    fi
                done

                INFO "已生成 diy_mount.txt: ${diy_mount_file}"
                INFO "内容预览："
                if [ -s "$diy_mount_file" ]; then
                    cat "$diy_mount_file" | while IFS=' ' read -r h c; do
                        [[ -n "$h" && -n "$c" ]] && echo "  - $h:$c"
                    done
                else
                    echo "  （无自定义挂载）"
                ...
                WARN "无法获取容器 /data 目录对应的宿主机路径，跳过生成 diy_mount.txt"
            fi
        fi
