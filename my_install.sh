...
fix_docker() {
    ...
    if echo skip pull # hello-world; then
...
function user_gbox() {
    ...
    local extra_volumes=""
    local diy_mount_file="$config_dir/diy_mount.txt"

    # 处理已存在的 diy_mount.txt
    if [ -s "$diy_mount_file" ]; then
        echo -e "\033[1;33m检测到配置目录中存在 diy_mount.txt 文件，包含自定义挂载配置。\033[0m"
        echo -e "${INFO}原文件内容："
        cat "$diy_mount_file" | while IFS=' ' read -r host_path container_path; do
            [[ -n "$host_path" && -n "$container_path" ]] && echo "  - $host_path:$container_path"
        done
        echo -e "${INFO}这些挂载将被添加到 G-Box 容器中。"
        read -erp "$(INFO "是否保留使用这些自定义挂载？（y/n）")" keep_mount

        if [[ $keep_mount == [Yy] ]]; then
            # 解析并验证 diy_mount.txt
            local temp_mounts=()
            local mount_valid=true
            ...
            while IFS=' ' read -r host_path container_path; do
                ...
                temp_mounts+=("-v $host_path:$container_path")
            done < "$diy_mount_file"
            ...
            if $mount_valid; then
                ...
                ERROR "原 diy_mount.txt 文件验证失败，已忽略该文件"
                extra_volumes=""
            fi
        else
            rm -f "$diy_mount_file"
            INFO "已删除原有的 diy_mount.txt 文件"
        fi
    fi

    # 询问是否添加新的自定义挂载
    echo -e "\033[1;33m是否添加其他自定义挂载？\033[0m"
    echo -e "${INFO}自定义挂载允许您将宿主机的目录或文件映射到 G-Box 容器中"
    echo -e "${INFO}例如：将宿主机的 /volume6/video 映射到容器的 /strm"
    read -erp "$(INFO "是否添加自定义挂载？（y/n）")" add_mount

    if [[ $add_mount == [Yy] ]]; then
        local reserved_paths=("/app" "/etc" "/sys" "/home" "/mnt" "/bin" "/data" "/dev" "/index" "/jre" "/lib" "/opt" "/proc" "/root" "/run" "/sbin" "/tmp" "/usr" "/var" "/www")

        while true; do
            echo -e "\n${INFO}请输入挂载配置，格式：宿主机路径:容器路径"
            echo -e "${WARN}示例：/volume6/video:/strm 或 /volume6/video:/strm:ro"
            read -erp "请输入（直接回车结束添加）：" mount_input
            ...
            local mount_opts=$(echo "$mount_input" | cut -d':' -f3-)

            [[ -z "$host_path" || -z "$container_path" ]] && {
                ERROR "输入格式错误，请使用格式：宿主机路径:容器路径"
                continue
            }

            # 检查宿主机路径是否存在
            if [ ! -e "$host_path" ]; then
                echo -e "${WARN}宿主机路径 $host_path 不存在"
                read -erp "$(INFO "是否创建该目录？（y/n）")" create_dir
                if [[ $create_dir == [Yy] ]]; then
                    if mkdir -p "$host_path" 2>/dev/null; then
                        INFO "已创建目录: $host_path"
                    else
                        ERROR "创建目录失败，请检查权限"
                        continue
                    fi
            ...
            # 检查容器路径是否是保留路径
            if [[ " ${reserved_paths[@]} " =~ " $container_path " ]]; then
                ERROR "容器路径 $container_path 是内部保留路径，不允许使用"
                continue
            fi

            # 构建挂载选项
            local mount_str="-v $host_path:$container_path"
            [[ -n "$mount_opts" && "$mount_opts" != "$container_path" ]] && mount_str="$host_path:$container_path:$mount_opts"

            # 添加到 extra_volumes
            extra_volumes+="$mount_str "

            # 添加到 diy_mount.txt（格式：宿主机路径 容器路径，用空格分隔）
            mkdir -p "$config_dir"
            echo "$host_path $container_path" >> "$diy_mount_file"

            INFO "已添加挂载: $host_path:$container_path"

            echo -e "\n${INFO}当前已添加的挂载："
            grep -v '^$' "$diy_mount_file" 2>/dev/null | while IFS=' ' read -r h c; do
                [[ -n "$h" && -n "$c" ]] && echo "  - $h:$c"
            done

            read -erp "$(INFO "是否继续添加挂载？（y/n）")" continue_add
...
choose_mirrors() {
    ...
    if [ ! -f "${config_dir}/docker_mirrors.txt" ]; then
        ...
        for i in "${!mirrors[@]}"; do
            ...
            if $success && echo skip pull # "${mirrors[$i]}/library/hello-world:latest" &> /dev/null; then
